...
ANNModel <- neuralnet(yyes ~ ., hidden = c(10, 5), data = tele_train, stepmax = 1e8, lifesign = "full", threshold = 0.05)
plot(ANNModel)
```
...
## ANN Model Analysis
Based on the ANN model that we created, we were able to reach an accuracy of 89.42% accuracy in predicting the outcomes of our test data   Based on the model, we are predicted to generate a profitability of $1126 if the call center were to reach out the customers that the model predicted would subscribe (only those that the model predicts to subscribe) to the campaign being promoted. This is calculated using the true positives and true negative test cases in the confusion matrix. The subscriber rate that we got was 54.65%.
